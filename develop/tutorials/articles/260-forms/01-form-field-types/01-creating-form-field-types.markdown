...
: The icon to be used for the field type. Choose one of the 
[Lexicon icons](https://lexicondesign.io/docs/patterns/icons.html) 
to make your form field blend with the existing form field types.
...
Next determine how your field type is rendered.
...
Extend `BaseDDMFormFieldRenderer`, an abstract class implementing the API's
only required method, `render`. The Form engine calls the render method for
every form field type present in a form, and returns the plain HTML of the
rendered field type. The abstract implementation also includes some utility
methods. Here's what the time field's `DDMFormFieldRenderer` looks like:
...
are a templating system for building UI elements. Liferay developers chose to
...
Create a file called `time_field.js` and give it these contents:
...
Next write the `*.es.js` file to configure the Soy template's rendering.

Create a file called `time.es.js` and populate it:
...
[tutorial on its usage](/develop/tutorials/-/knowledge_base/7-0/liferay-amd-module-loader).
